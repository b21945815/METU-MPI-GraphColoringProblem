...
void Algorithm1HalfAsynchronous::parallelGreedyColoring(vector<vector<int>>& graph, int rank, int size, vector<int>& colors) {
    int n = graph.size();
...
bool Algorithm1HalfAsynchronous::detectConflicts(vector<vector<int>>& graph, int rank, int size, vector<int>& colors, vector<int>& conflicts, bool resolveSmall) {
    int n = graph.size();
    int blockSize = n / size;
...
void Algorithm1HalfAsynchronous::resolveConflictsParallel(vector<vector<int>>& graph, int rank, int size, vector<int>& colors, vector<int>& conflicts) {
    int n = graph.size();
    for (int u : conflicts) {
        if (u == -1) continue;
...
void Algorithm1HalfAsynchronous::algorithm1HalfAsynchronous(vector<vector<int>>& graph) {
    ...
    if (rank == 0) {
        ...
        std::string filename = "C:\\Users\\fatih\\source\\repos\\TRIAL_1\\Results\\algorithm1halfAsynchronous_" + std::to_string(size) + ".txt";
...
void Algorithm1HalfAsynchronous::runAlgorithm1HalfAsynchronous() {
    int rank, size;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &size);
    if (rank == 0) {
        ...
        std::ofstream outfile(filename, std::ios::app);
        if (outfile.is_open()) {
            outfile << "Algorithm 1 Half Asynchronous Started ..." << endl;
        }
        else {
            cerr << "Error: Unable to open file for writing!" << endl;
        }
    ...
    algorithm1HalfAsynchronous(graph);
    if (rank == 0) {
    ...
    if (rank == 0) {
        ...
        std::ofstream outfile(filename, std::ios::app);
        if (outfile.is_open()) {
            outfile << "Algorithm 1 Half Asynchronous Finished!" << endl;
            outfile << "======================================" << endl;
        }
        else {
            cerr << "Error: Unable to open file for writing!" << endl;
